#!/bin/bash
# pyvar.com Phase P4 — UNIFIED v3
# AWS Deployment — CDK All Stacks | Wk 11-15 | Agent Teams: ⚠️  CONDITIONAL
# Usage: ./phase-p4.sh [--dry-run|--status|--at-only]
set -euo pipefail
...
echo "╔══════════════════════════════════════════════════════╗"
echo "║ pyvar · P4: AWS Deployment — CDK All Stacks            ║"
echo "║ Wk 11-15   · Agent Teams: ⚠️  CONDITIONAL      ║"
echo "╚══════════════════════════════════════════════════════╝"
echo ""
# AWS CDK: aws-architect, cloud-infra, docker, terraform.
# Agent Teams CONDITIONAL: safe for cdk synth / smoke tests. UNSAFE for cdk deploy.
echo ""
...
run claude plugins enable terraform-patterns@claude-code-skills
...
echo ""
echo "✓ P4 done: 13 enabled | 35 disabled | 2 never"
echo "  10 original | 1 wshobson | 2 zip-new | AT: ⚠️  CONDITIONAL"
echo "  Local SKILL.md files: always active (no toggle needed)"
echo "  Start: claude --dangerously-skip-permissions --model claude-opus-4-6"
echo ""
