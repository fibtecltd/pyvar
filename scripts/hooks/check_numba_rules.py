...
def notify(violations: list[Violation]) -> None:
    ...
    print(
        f"\n{BOLD}{CYAN}━━ Numba JIT Rule Checker ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}",
        file=sys.stderr,
    )
    ...
    for v in violations:
        ...
        print(
            f"{colour}{v.severity}{RESET}  [{v.file}:{v.line}] " f"RULE {v.rule}: {v.message}",
            file=sys.stderr,
        )
...
class NumbaRuleVisitor(ast.NodeVisitor):
        ...
        for child in ast.walk(node):
            ...
            if isinstance(child, ast.Call):
                ...
                if isinstance(func, ast.Name) and func.id in ("hasattr", "getattr", "setattr"):
                    self._add(
                        2, child, f"{fname}: {func.id}() inside @njit — no dynamic dispatch allowed"
                    )
    ...
    def visit_Import(self, node: ast.Import) -> None:
        if self._current_njit:
            self._add(1, node, f"{self._current_njit}: import statement inside @njit function")
        self.generic_visit(node)
    ...
    if "lifespan" not in text:
        ...
        violations.append(
            Violation(
                ...
                "ECS health check startPeriod=30s depends on this.",
                severity="WARNING",
            )
        )
    return violations
