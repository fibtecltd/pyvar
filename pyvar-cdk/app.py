...
network = NetworkStack(app, f"{prefix}-network", cfg=cfg, env=env_primary,
    description="pyvar: VPC, subnets, security groups, VPC endpoints")

data = DataStack(app, f"{prefix}-data", cfg=cfg,
    vpc=network.vpc, sgs=network.sgs, env=env_primary,
    description="pyvar: Aurora SV2, ElastiCache Serverless, S3")

queue = QueueStack(app, f"{prefix}-queue", cfg=cfg, env=env_primary,
    description="pyvar: SQS FIFO job queue + DLQ + CloudWatch alarms")

compute = ComputeStack(app, f"{prefix}-compute", cfg=cfg,
    vpc=network.vpc, sgs=network.sgs,
    var_queue=queue.var_queue, dlq=queue.dlq, data=data,
    env=env_primary,
    description="pyvar: EC2 Spot ASG Celery workers + step scaling")

api = ApiStack(app, f"{prefix}-api", cfg=cfg,
    vpc=network.vpc, sgs=network.sgs,
    var_queue=queue.var_queue, data=data,
    env=env_primary,
    description="pyvar: ECS Fargate FastAPI + ALB + auto-scaling")

edge = EdgeStack(app, f"{prefix}-edge", cfg=cfg,
