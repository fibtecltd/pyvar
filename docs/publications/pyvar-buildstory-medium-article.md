...
Somewhere in the first draft of pyvar's Solvency II module, one compounding factor went missing. It was a small defect — a few missing lines in one formula. The consequence wasn't small: an insurer running it would have been told it needed roughly 79% less regulatory capital than Solvency II actually requires.
...
pyvar.com is an open-source (Apache-2.0) engine for exactly this kind of high-stakes number — 385 functions covering Value-at-Risk, Expected Shortfall, credit risk, derivatives Greeks, and regulatory capital across Basel, Solvency II, EMIR, and more. What makes it worth writing about isn't the domain coverage on its own. It's that the whole thing — math kernels, deployment pipeline, and the validation suite that caught the error above — was built end-to-end by Claude Code.

## At a glance

![At a glance: 385 functions across 8 domains, 649 commits with 131 co-authored by Claude, 4 regulatory errors caught pre-launch, 23.6% caveats disclosed inline, Apache-2.0 licensed](./glance_strip.png)
...
> defect description in this article is checkable against this repository —
...
depends on them. If a bank's SCR formula has a defect, nobody outside the
...
kernels, found real regulatory defects before they shipped, built the CI/CD
...
## What Claude Code's own validation caught before anyone else could
...
surfaced a real defect in the Monte Carlo CVaR optimizer's solver — again,
...
so the flaw and the correct behaviour produced identical numbers in that
...
real `pd` of 1–8%), so the old flaw would fail this test loudly if it ever
came back.
...
## The Jupyter integration: three defects a naive build would have shipped
...
discipline that finds a formula error — don't trust it, run it and check —
...
II/EMIR/IRRBB defects before launch, build and harden the AWS deployment
