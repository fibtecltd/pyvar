...
| 5 | Monetization strategy implementation for pro/enterprise tiers | High | Confirmed: tier enforcement already works in production (free/pro/enterprise rate limits, `User.tier` schema "denormalised for billing queries"), but there is genuinely **no way for anyone to become pro/enterprise** and **zero payment integration** anywhere in the codebase. **Phase A shipped this session**: Stripe Checkout + webhook (`api/routes/billing.py`), confirmed decisions (Stripe/Phase-A-only/monthly-only/no-trial/manual-Enterprise), 14 new tests + full 991-test suite re-run clean. Also closed a JWT-staleness gap the original plan didn't anticipate (an already-issued JWT's tier claim never re-checks the DB). Deliberately NOT done: wiring the 3 Stripe secrets into the live `pyvar-{env}-api` ECS task (real deploy risk this session can't verify — exact CDK snippet + ordering left in the plan doc) or a real end-to-end Stripe test-mode run | `docs/plan-monetization-implementation.md` (this session) |
